...
- `Client::with_config` 配 `http2: true` 即用明文 h2c（TCP 直连的 HTTP/2 前导知识），本例刻意不涉及 TLS。HTTPS 在配置 `ClientConfig::tls` 后使用内置 TLS 1.3；HTTP/3 使用相同 TLS 路径的 `h3` ALPN，并设置 `ClientConfig::http3: true`。
