...
- **RFC 8441 (WebSocket over HTTP/2) is not implemented.** A client that is offered `h2` by ALPN fails with a clear error rather than opening a connection that looks established and never carries a frame. Use HTTP/1.1 for WebSockets, as almost everyone does.
...
See [`examples/ws_echo.rs`](../../examples/ws_echo.rs) and [`examples/ws_client.rs`](../../examples/ws_client.rs) for runnable versions, and [`tests/ws.rs`](../../tests/ws.rs) for 25 end-to-end tests that exercise the wire protocol against a real socket, including cross-driver push, origin rejection, wss over TLS, and the error codes.
