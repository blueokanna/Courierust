...
pub fn serve(stream: TcpStream, handler: &dyn Handler, config: &ServerConfig) -> Result<()> {
    ...
    loop {
        ...
        let headers = h1::read_headers(&mut reader)?;
        let body = match h1::body_length(&headers, Some(&rl.method), None)? {
            h1::BodyLen::None => Body::Empty,
            h1::BodyLen::Length(n) => {
                Body::Bytes(h1::read_body_fixed(&mut reader, n, config.max_body)?)
            }
            h1::BodyLen::Chunked => {
                Body::Bytes(h1::read_body_chunked(&mut reader, config.max_body)?)
            }
        };
        let req = Request {
            method: rl.method,
        ...
        if chunked {
            ...
            out_headers.insert(
                ...
                HeaderValue::from_bytes(n.to_string().as_bytes())?,
            );
        ...
        h1::write_response_head(&mut head, resp.status, Version::HTTP_11, &out_headers)?;
