# Courierust 工程故事：四个"看不见"的设计

这个系列想讲四件事。它们都不产生任何用户可见的数据，但决定了你的 HTTP 栈**快不快、稳不稳、扛不扛得住**。

| 篇 | 主题 | 一句话 | 生活类比 |
|---|---|---|---|
| [01](./01-wucs-scheduler.md) | HTTP/2 优先级调度器 | 一个让大家都放弃的功能，我用 30 行代码做成了 O(1) | 医院急诊分诊：危重优先，但感冒病人也饿不死 |
| [02](./02-bcr-flow-control.md) | 批量信用回流 | 我数了一下控制帧，吓了一跳，然后砍掉了一个数量级 | 送水工：剩半桶水就预约，永远不断水 |
| [03](./03-self-pipe-event-scheduler.md) | 事件调度器 | 一个 5ms 的 P99 尖峰，逼我写了个"烧水壶哨子" | 餐厅前台：不每 5 分钟看一次铃，而是装个哨子 |
| [04](./04-h3-ack-deadline.md) | H3 的 ACK 与 deadline | 一个写在注释里、却从没生效的快速路径，把 H3 拖进 5ms 台阶 | 前台说好"上菜就按铃"，结果铃是个摆设，客人干等下一轮叫号 |

## 它们解决的是同一个问题的几个侧面

一条 HTTP 连接上，同时发生着几件事：**谁先发**（调度）、**什么时候还信用**（流控）、**空闲时怎么不浪费资源**（事件循环）、**事件到了之后多久被处理**（唤醒/deadline）。

```mermaid
flowchart LR
    A["WUCS<br/>谁先发"] --> D["每帧 O(1) 的决策"]
    B["BCR<br/>何时还信用"] --> E["控制帧少一个数量级"]
    C["Self-pipe<br/>怎么不浪费"] --> F["空闲连接零 worker 占用"]
    H["H3 ACK/deadline<br/>事件何时被处理"] --> I["ack 立即回、poll 睡到 deadline"]

    D --> G["快"]
    E --> G
    F --> G["扛得住"]
    I --> G
```

- **WUCS** 回答：一堆流抢带宽，这一帧给谁？——高优先级优先，低优先级饿不死，每帧都付得起。
- **BCR** 回答：收到数据，什么时候把信用还给发送方？——攒批还，别攒到空。
- **Self-pipe** 回答：几千条空闲连接怎么不占资源？控制消息怎么不排队等 tick？——挂轮询器 + 一个能吵醒 poll 的哨子。
- **H3 ACK/deadline** 回答：事件到了之后多久被处理？——ACK 走交互式快速路径立即回，poll timeout 是绝对协议 deadline，不是固定节拍。同一个"别等下一个 tick"教训，在 H3 里以 ACK 政策 + deadline 折叠的形式又解决了一次。

## 关于 Courierust

[Courierust](https://github.com/blueokanna/Courierust) 是一个零第三方依赖、`no_std + alloc` 可编译、全程无 `unsafe` 的 HTTP/1.1 + HTTP/2 + gRPC 协议栈，并自带从零实现的 TLS 1.2/1.3 与 QUIC v1 / HTTP/3。本系列讲的就是其中三个"不起眼但全是设计"的角落。
